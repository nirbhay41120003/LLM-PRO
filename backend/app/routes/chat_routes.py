...
def health_chat():
    """
    Handle health-related queries using BioBERT + RAG
    ...
    try:
        ...
        if health_model is None:
            return jsonify({
                ...
                'response': 'The health model is not initialized. Please run the training pipeline first.'
            }), 503
        
        response_data = health_model.get_health_response(user_message)
        
        # Add severity analysis if available
        if hasattr(health_model, 'analyze_symptom_severity'):
            severity = health_model.analyze_symptom_severity(user_message)
            response_data['severity_analysis'] = severity
...
def analyze_symptoms():
    """
    Analyze symptoms and return detailed disease prediction
    ...
    try:
        ...
        if health_model is None:
            return jsonify({'error': 'Health model not available'}), 503
        
        # Get prediction
        prediction = health_model.predict(symptoms)
        
        # Get severity analysis
        severity = health_model.analyze_symptom_severity(symptoms)
...
def rag_search():
    ...
    try:
        rag_system = current_app.rag_system
        
        if rag_system is None:
            return jsonify({'error': 'RAG system not available'}), 503
        
        # Search knowledge base
        results = rag_system.knowledge_base.search(query, top_k=top_k)
...
def general_chat():
    """Handle general queries - redirects to health chat for medical queries"""
    data = request.get_json()
    ...
    try:
        # Use health model for all queries - it handles both symptoms and general health questions
        health_model = current_app.health_model
        ...
        return jsonify({
            'response': 'I am a health assistant. Please describe your symptoms or ask a health-related question.'
        })
    except Exception as e:
